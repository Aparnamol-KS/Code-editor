...
function Attempt() {
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            setOutput(response.data.result || "Submission done.");
            console.log(output)
        } catch (err) {
        ...
        <div className="h-screen w-screen bg-[#0d1117] text-white font-['JetBrains_Mono'] overflow-hidden">
            <header className="flex justify-between items-center px-6 py-2 bg-[#161b22]/95 backdrop-blur-sm border-b border-gray-700 shadow-lg sticky top-0 z-50">
                ...
                <div className="flex gap-3 items-center">
                    ...
                    <button
                        className="bg-gradient-to-r from-[#0090FF] to-[#007BFF] hover:from-[#1d4ed8] hover:via-[#3b82f6] hover:to-[#38bdf8] transition-all px-5 py-2 rounded-md font-semibold text-white shadow-md hover:shadow-lg"
            ...
            <Split className="flex h-[calc(100%-60px)]" sizes={[40, 60]} minSize={300} gutterSize={8}>
                <div className="bg-[#0d1117] border-r border-gray-700 overflow-y-auto">
                    <div className="sticky top-0 bg-[#0d1117]/95 backdrop-blur-sm z-10 p-5 shadow-md">
                        <h2 className="text-3xl font-semibold font-['Share_Tech'] text-gray-300">{problem.title}</h2>
                        <span className={`${problem.difficulty === "Easy" ? "text-green-400" : problem.difficulty === "Medium" ? "text-yellow-400" : "text-red-400"}`}>{problem.difficulty}</span>
                    </div>
                    ...
                    <div className="p-5 space-y-4 text-gray-400 leading-relaxed">
                        ...
                        <div className="mt-6">
                            <h3 className="text-lg font-['Rajdhani'] font-semibold mb-2 text-gray-300">Sample Test Cases :</h3>
                            {problem.testcases?.length > 0 ? (
                                <div className="space-y-3">
                                    {problem.testcases.map((testcase, index) => (
                                        <div key={index} className="p-3 bg-gray-800 border border-gray-700 rounded-lg">
                                            <p className="text-gray-300 font-['JetBrains_Mono']"><span className="text-cyan-400">Input:</span> {testcase.input}</p>
                                            <p className="text-gray-300 font-['JetBrains_Mono']"><span className="text-cyan-400">Expected Output:</span> {testcase.expectedOutput}</p>
                                        </div>
                                    ))}
                                </div>
                            ) : (
                ...
                <Split direction="vertical" className="flex flex-col h-full" sizes={[70, 30]} minSize={200} gutterSize={8}>
                    <div className="bg-[#0d1117] border-b border-gray-700 overflow-y-auto relative">
                        <div className="sticky top-0 bg-[#0d1117]/95 backdrop-blur-sm z-10 pt-5 px-5 shadow-md">
                            <h2 className="text-2xl font-semibold font-['Share_Tech'] text-gray-300">Code Editor</h2>
                        </div>
            ...
            <pre className="text-gray-400 text-sm whitespace-pre-wrap">
                ...
                {output ? (
                    <>
                        ...
                        >
                            {output.passedTestCase}
                        </span>
                        <span className="text-gray-400"> / {output.total}</span>
                    </>
                ) : (
